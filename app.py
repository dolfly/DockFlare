...
def cf_api_request(method, endpoint, json_data=None, params=None):
    ...
    try:
        ...
        if e.response is not None:
            ...
            logging.error(f"Response Body: {error_data}"); cf_errors = error_data.get('errors', [])
    ...
    try:
        ...
        if response_data and isinstance(response_data, dict) and response_data.get("success"):
            ...
            if isinstance(result_data, dict):
                 ...
                 if isinstance(config_data, dict): logging.debug(f"Successfully fetched and parsed config: {config_data}"); return config_data
                 else: logging.info(f"Fetched config is null/empty. Response: {response_data}"); return {}
            else: logging.warning(f"API resp success but no 'result' dict. Resp: {response_data}"); return {}
        elif response_data: logging.warning(f"API resp format unexpected/no success. Resp: {response_data}"); return {}
        else: logging.error("cf_api_request returned None for GET config."); return None
    except Exception as e:
        logging.error(f"Exception in get_current_cf_config: {e}", exc_info=True)
        if not tunnel_state.get("error") or "API Error" not in tunnel_state["error"]: tunnel_state["error"] = f"Failed get/parse tunnel config: {e}"
        return None
...
def update_cloudflare_config():
    ...
    with state_lock:
        ...
        for hostname, rule_details in managed_rules.items():
            if rule_details.get("status") == "active": # Check status exists and is active
                desired_rule = {"hostname": hostname, "service": rule_details.get("service")}
                if not desired_rule["service"]: logging.warning(f"Rule for {hostname} is active but missing service details. Skipping."); continue # Skip invalid active rule
                found_in_cf = False
                for existing_rule in current_config.get("ingress", []):
                    if existing_rule.get("hostname") == hostname:
                        found_in_cf = True
                        if existing_rule.get("service") != desired_rule["service"]: logging.info(f"Updating service for {hostname}."); changed = True
                        break
        ...
        for existing_rule in current_config.get("ingress", []):
             if existing_rule.get("service") != catch_all_rule["service"]:
                 if not existing_rule.get("hostname") or existing_rule.get("hostname") not in managed_rules:
                      logging.debug(f"Keeping unmanaged rule: {existing_rule}"); new_ingress_rules.append(existing_rule)

        cf_hostnames_present_in_active_state = {r for r, d in managed_rules.items() if d.get('status') == 'active'}
        if current_hostnames_in_cf - cf_hostnames_present_in_active_state:
             logging.info(f"Detected hostnames in CF to be removed: {current_hostnames_in_cf - cf_hostnames_present_in_active_state}"); changed = True
        ...
        if not changed and set(map(rule_to_tuple, current_cf_rules_no_catchall)) != set(map(rule_to_tuple, new_rules_no_catchall)):
            logging.info("Detected difference in rule sets."); changed = True
...
def process_container_start(container):
    ...
    try:
        ...
        enabled = labels.get(f"{LABEL_PREFIX}.enable", "f").lower() == "t"
        ...
        with state_lock:
            ...
            if existing_rule:
                if existing_rule.get("status") == "pending_deletion": logging.info(f"Cancelling pending deletion for {hostname}."); existing_rule["status"] = "active"; existing_rule["delete_at"] = None; needs_update = True; state_changed = True
                if existing_rule.get("service") != service or existing_rule.get("container_id") != container_id: logging.info(f"Updating service/container for {hostname}."); existing_rule["service"] = service; existing_rule["container_id"] = container_id; state_changed = True # Check if this change requires CF update
            else: logging.info(f"Adding new rule for {hostname}."); managed_rules[hostname] = {"service": service, "container_id": container_id, "status": "active", "delete_at": None}; needs_update = True; state_changed = True

        if needs_update:
             if update_cloudflare_config():
                 if state_changed: save_state()
             else: logging.error(f"Failed update CF config for start {hostname}.")
        elif state_changed: save_state()
...
def schedule_container_stop(container_id):
    ...
    with state_lock:
        ...
        if hostname_to_schedule:
            logging.info(f"Marking hostname {hostname_to_schedule} for deletion."); rule = managed_rules[hostname_to_schedule]
    ...
    try:
        ...
        for event in events:
            ...
            if ev_type == "container" and cont_id:
                ...
                elif action in ["stop", "die", "destroy"]: schedule_container_stop(cont_id)
                ...
                if update_cloudflare_config():
                    ...
                    with state_lock:
                        for hostname in hostnames_to_remove:
                            if hostname in managed_rules and managed_rules[hostname].get("status") == "pending_deletion": del managed_rules[hostname]; state_changed_in_cleanup = True
    ...
    try:
        with state_lock:
            ...
            try:
                 ...
                 for c in containers: labels=c.labels; enabled=labels.get(f"{LABEL_PREFIX}.enable","f").lower()=="t"; hn=labels.get(f"{LABEL_PREFIX}.hostname"); svc=labels.get(f"{LABEL_PREFIX}.service");
                 if enabled and hn and svc: running_labeled_containers[hn]={"service":svc,"container_id":c.id,"container_name":c.name}
            ...
            cf_ingress_hostnames = {r.get("hostname") for r in current_cf_config.get("ingress", []) if r.get("hostname")}
            ...
            for hostname, running_details in running_labeled_containers.items():
                ...
                if hostname in managed_rules:
                    ...
                    if rule.get("status") == "pending_deletion": logging.info(f"[Reconcile] Reactivating {hostname}."); rule["status"] = "active"; rule["delete_at"] = None; needs_cf_update = True; state_changed = True
                    if rule.get("service") != running_details["service"] or rule.get("container_id") != running_details["container_id"]: logging.info(f"[Reconcile] Updating state for {hostname}."); rule["service"] = running_details["service"]; rule["container_id"] = running_details["container_id"]; state_changed = True; needs_cf_update = True # Need update if service changed
            ...
            for hostname, rule in list(managed_rules.items()):
                 if hostname not in hostnames_processed: hostnames_processed.add(hostname)
                 if rule.get("status") == "active" and hostname not in running_labeled_containers: logging.info(f"[Reconcile] Scheduling deletion for {hostname} (not running)."); rule["status"] = "pending_deletion"; rule["delete_at"] = now + timedelta(seconds=GRACE_PERIOD_SECONDS); state_changed = True; needs_cf_update = True
                 elif rule.get("status") == "pending_deletion" and hostname in running_labeled_containers: logging.info(f"[Reconcile] Reactivating {hostname} (pending but running)."); rule["status"] = "active"; rule["delete_at"] = None; needs_cf_update = True; state_changed = True
            if needs_cf_update or state_changed:
                 logging.info(f"[Reconcile] State/CF sync needed. Triggering update.")
                 if update_cloudflare_config():
                     if state_changed: save_state()
...
def start_cloudflared_container():
    ...
    try:
        ...
        if container:
            if container.status == 'running': msg = f"Container '{CLOUDFLARED_CONTAINER_NAME}' already running."; logging.info(msg); cloudflared_agent_state["last_action_status"] = msg; success_flag = True; return True
            else: logging.info(f"Starting existing '{CLOUDFLARED_CONTAINER_NAME}'..."); container.start(); msg = f"Started existing '{CLOUDFLARED_CONTAINER_NAME}'."; cloudflared_agent_state["last_action_status"] = msg; logging.info(msg); success_flag = True
        else:
            logging.info(f"Container '{CLOUDFLARED_CONTAINER_NAME}' not found. Creating...")
            ...
            msg = f"Created and started container '{new_container.name}'."; cloudflared_agent_state["last_action_status"] = msg; logging.info(msg); success_flag = True
    except APIError as e: msg = f"Docker API error during start: {e}"; logging.error(msg, exc_info=True); cloudflared_agent_state["last_action_status"] = f"Error: {msg}"; success_flag = False
        ...
        if docker_client: logging.debug("Updating container status after start attempt."); time.sleep(2); update_cloudflared_container_status() # Add delay before status update
...
def stop_cloudflared_container():
    ...
    try:
        ...
        if container.status != 'running': msg = f"Container '{CLOUDFLARED_CONTAINER_NAME}' not running (status: {container.status})."; logging.info(msg); cloudflared_agent_state["last_action_status"] = msg; success_flag = True; return True
        ...
        if docker_client: logging.debug("Updating container status after stop attempt."); time.sleep(2); update_cloudflared_container_status() # Add delay
...
def start_tunnel():
    start_cloudflared_container(); return redirect(url_for('status_page'))

@app.route('/stop', methods=['POST'])
def stop_tunnel():
    stop_cloudflared_container(); return redirect(url_for('status_page'))
...
def force_delete_rule(hostname):
    ...
    state_changed = False; update_success = False
    with state_lock:
        if hostname in managed_rules:
            logging.info(f"Removing rule for {hostname} from local state."); del managed_rules[hostname]; state_changed = True
        else:
            logging.warning(f"Attempted force delete for {hostname}, not found in state.")
    ...
    if state_changed:
        ...
        if update_cloudflare_config():
            ...
            save_state() # Save state ONLY if CF update succeeded
            cloudflared_agent_state["last_action_status"] = f"Successfully force deleted rule for {hostname}."
        else:
            logging.error(f"CRITICAL: Failed CF update after removing {hostname} from state. State inconsistent!")
            cloudflared_agent_state["last_action_status"] = f"Error: Failed pushing force delete for {hostname} to Cloudflare!"
            # DO NOT save state here, let reconciliation fix it later
    return redirect(url_for('status_page'))

# --- Main Execution ---
if __name__ == '__main__':
    logging.info("Application starting up..."); load_state(); logging.info("State loading complete.")
    ...
    if tunnel_state.get("id") and tunnel_state.get("token"):
         logging.info("Tunnel initialized. Proceeding with reconciliation/agent start.")
    ...
    else: logging.warning("Tunnel not fully initialized, skipping agent start/background tasks.")

...
2.  **Create and Configure `.env`:**
    ...
    EXTERNAL_HOSTNAME=example.com # Set the external hostname for reverse proxy (e.g., myapp.example.com)
    BASE_URL=/ # Set the base URL if the app is served under a subpath (e.g., /dockflare)
    ```
    ...
    *   `EXTERNAL_HOSTNAME`: (Optional) Set the external hostname for reverse proxy (e.g., `myapp.example.com`).
    *   `BASE_URL`: (Optional) Set the base URL if the app is served under a subpath (e.g., `/dockflare`).
...
If you are running DockFlare behind a reverse proxy, you can configure the following environment variables:

- `EXTERNAL_HOSTNAME`: Set this to the external hostname of your application (e.g., `myapp.example.com`). This ensures that URLs generated by the application are correct.
- `BASE_URL`: Set this to the base path under which the application is served (e.g., `/dockflare`). This is useful if the reverse proxy serves the application under a subpath.

Example `.env` configuration:

```dotenv
EXTERNAL_HOSTNAME=myapp.example.com
BASE_URL=/dockflare
```

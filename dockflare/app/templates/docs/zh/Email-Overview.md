...
Cloudflare Email Sending（Beta）根据您的 Cloudflare 套餐提供两个级别：

| 收件方 | 免费套餐 | Workers 付费套餐（5 美元/月） |
| :--- | :--- | :--- |
| 已验证的 Cloudflare 地址（在您 CF 账户中确认的地址） | ✅ 允许 | ✅ 允许 |
| 任意外部地址 | ❌ 不允许 | ✅ 允许 |

DockFlare 在域名设置期间会自动配置 DKIM 签名记录和发送子域名（`mail.yourdomain.com`）。但是，**完整的外部发送需要额外两个手动步骤：**

1. **升级到 Cloudflare Workers 付费套餐** — 在 Cloudflare 控制台中每月 5 美元即可开通。
2. **激活 CF Email Sending（Beta）** — 前往 [Cloudflare 控制台 → Email → Email Sending](https://dash.cloudflare.com/) 并为您的账户启用该功能。

在完成以上步骤之前，从网页邮件客户端发出的邮件仅能投递到您 Cloudflare 账户中已验证的电子邮件地址。DockFlare 邮件管理页面上的域名状态徽章反映 DKIM 是否已配置（`Sending: Active`）或尚未设置（`Sending: Pending`）。
